...
export interface SearchDialogProps {
   ...
   * carried out, so the window can say why. Falls back to a location search
...
/**
 * Location search and, where a project enables it, the natural-language command
 * bar — as a modal window.
 *
 * Same shell as BasemapDialog ("Referentielagen") and LayerMetaDialog: these are
 * the app's "chrome" dialogs and are meant to read as one family, down to the
 * uppercase heading and the ghost close button.
 *
 * It was a popover pinned beside the search button. A modal suits it better for
...
export function SearchDialog(props: SearchDialogProps): JSX.Element {
  ...
  /**
   * Whether this is a command bar at all. The geocoding path is unchanged either
   * way — `onCommand` falls back to a location search whenever the model is
   * missing, still downloading, or fails.
   */
  ...
  function handleKeyDown(e: KeyboardEvent) {
    ...
    // Escape closes the list first and the window only once it is already
    // empty, so a stray keypress never discards a query mid-search. Stopped
    // from bubbling in that case, or the dialog shell would close it anyway.
  ...
  /** Drop everything typed so far, so a reopened window starts clean. */
      ...
      <DialogContent class="app-scrollbar top-[20vh] max-h-[calc(80vh-1rem)] w-[min(40rem,calc(100vw-2rem))] translate-y-0">
        <div class="mb-5 flex items-center justify-between gap-2">
          ...
          <DialogTitle class="text-xs font-semibold uppercase tracking-wide text-gray-500">
            {commandMode() ? "Zoeken of opdracht geven" : "Zoeken"}
          </DialogTitle>
          <Button
            variant="ghost"
            size="icon-sm"
        ...
        <form onSubmit={handleSearch} class="flex flex-col gap-2">
          ...
          {/* Place candidates for what is typed so far. An ambiguous name —
              "Bergen" is three different gemeenten — is picked from here rather
              than guessed at. */}
          <Show when={suggestions().length > 0}>
            <ul
              id="map-search-suggestions"
              role="listbox"
              class="max-h-72 overflow-y-auto rounded border border-gray-200 py-0.5"
            >
          ...
          {/* Why a command could not be carried out. Only ever rendered when
              there is something to say, so the window keeps its height in the
              ordinary case. */}
          <Show when={message()}>
            {(text) => <p class="px-1 text-xs text-gray-600">{text()}</p>}
          </Show>
        </form>
      </DialogContent>
    </DialogRoot>
