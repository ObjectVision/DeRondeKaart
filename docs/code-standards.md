...
The conventions this codebase already follows. Match them.

The `code-review` skill reads this file as its Standards axis. Three rules fail **silently** and are repeated in `AGENTS.md`: never destructure Solid props, read reactive inputs before an early return, and English code with Dutch user-facing strings.
